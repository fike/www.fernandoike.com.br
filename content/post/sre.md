...
Há também a liberdade do profissional SRE de mudar de equipe por alguma razão. Geralmente, a saída de um membro de uma equipe na Operação impacta na atividade dos outros da equipe. No Google, este impacto  é relativamente pequeno porque todas equipes usam os mesmos processos e princípios. Por exemplo, se um pessoa decidir mudar de equipe porque quer um novo desafio mas ele não tem conhecimento do framework usado pela outra equipe, isso não é um impeditivo para mudança já que os processo são os mesmo, ele aprenderá a rotina e o framework rapidamente. Outro ponto interessante é que no processo de recrutamento de novos SRE, não é requisito saber as linguagens e frameworks usados pelo Google, o mais relevante é que o candidato saber programar (**Coders**).
...
As equipes SREs não são equipes isoladas, elas se relacionam com diversas outras equipes. Além as equipes de produtos, eles também tem forte interação com os
...
Abaixo destaco alguns que merecem serem destacados.
...
Uma das pessoas da equipe disse uma vez (infelizmente não quem foi) - "*As informações do monitoramento só são úteis se as pessoas ou robôs souberem o que fazer fazer com elas.*
...
Os tipo de testes são variados, desde os mais conhecidos como: Testes Unitários e Testes de Integração à outros como Testes de Regressão e Testes de Configuração. Ah, sim! Meus tipos de testes favoritos: Testes de Stress e Testes de Desastres.
